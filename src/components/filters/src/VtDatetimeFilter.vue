...
import VtDatetimeFilterMore from "./VtDatetimeFilterMore.vue";
...
const filteredString = computed(() => {
	if (filtered.value !== true) {
		...
		if (moreFiltered.value === true) {
			return datetimeFormat(filterMoreString.value, props.dateType);
		} else {
			return {
				groupOp: "and",
				rules: [
					{
						...
						op: "eq",
						data: datetimeFormat(filterString.value, props.dateType)
					}
				]
			};
		}
...
function onCancelFilterMore() {
	...
	if (filterButton != null) {
		...
		filterMoreString.value =
			prevFilterString == null ? null : datetimeFormat(prevFilterString, props.dateType);
	}
}
