...
const filterResult = computed(() => {
	if (!filterCondition1.value.value && !filterCondition2.value.value) {
		return null;
	} else if (filterCondition2.value.value) {
		return {
...
watch(
	...
	(newValue) => {
		if (newValue && filterCondition2.value.operation.length == 0) {
...
watch(
	...
	(newValue) => {
		if (newValue === true) {
			...
			filterCondition1.value.value = undefined;
			filterCondition2.value.operation = "";
			filterCondition2.value.value = undefined;
			...
			if (initValue && initValue.rules) {
				...
				if (initValue.rules.length > 0) {
					...
					filterCondition1.value.value = isNaN(firstCondition.data)
						? undefined
						: Number(firstCondition.data);
				}
				if (initValue.rules.length > 1) {
					const secondCondition = initValue.rules[1];
					filterCondition2.value.operation = secondCondition.op;
					filterCondition2.value.value = isNaN(secondCondition.data)
						? undefined
						: Number(secondCondition.data);
				}
			}
		}
	}
	...
	>
		...
		<div class="flex">
			...
			<div class="flex-initial w-64">
				<el-input-number v-model="filterCondition1.value" />
			</div>
		</div>
		...
		<div class="flex">
			...
			<div class="flex-initial w-64">
				<el-input-number v-model="filterCondition2.value" />
			</div>
		</div>
		<template #footer>
			<div class="mr-2">
